...
let fileContent = fs.readFileSync('src/components/CinematicGallery.tsx', 'utf8');
...
fileContent = fileContent.replace(/const MEDIA: VideoMedia\[\] = \[\s*\{[\s\S]*?\];/m, newMedia);

fs.writeFileSync('src/components/CinematicGallery.tsx', fileContent);
console.log('CinematicGallery updated');
